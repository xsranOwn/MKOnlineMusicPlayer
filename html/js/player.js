...
// 播放器功能配置
var mkPlayer = {
    api: location.protocol + '//' + location.host + '/api/', // api地址（自动适应域名）
...
// 存储全局变量
var rem = [];
rem.api = mkPlayer.api;  // API 地址（自适应域名）
...
function play(music) {
    ...
    // MediaSession（在设置 metadata 后立即注册 action handler，确保系统控件激活）
    if ("mediaSession" in navigator) {
        var artworkUrl = music.pic;
        if (!artworkUrl || artworkUrl === "err") {
            artworkUrl = window.location.origin + "/images/player_cover.png";
        }
...
// MetaData接入（在 play() 中设置 metadata 时注册）
